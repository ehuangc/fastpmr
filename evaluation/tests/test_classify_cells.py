...
# via a session-scoped fixture and have each parametrized case look up its
# result.
CASES = [
    # ── Original cases ──
    (40, 80, "asia"),  # Xinjiang
...
def classified() -> dict[tuple[float, float], str]:
    coords = [(lat, lon) for lat, lon, _ in CASES]
    regions = classify_coords(coords)
    return dict(zip(coords, regions, strict=True))


@pytest.mark.parametrize("lat, lon, expected_region", CASES)
def test_classify_coords(
    lat: float,
    lon: float,
    expected_region: str,
    classified: dict[tuple[float, float], str],
) -> None:
    region = classified[(lat, lon)]
