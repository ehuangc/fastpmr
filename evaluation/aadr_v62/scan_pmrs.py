...
AADR_V62_METADATA_PATH = AADR_V62_DIR / "data" / "v62.0.p1.1240k.aadr.anno"

# v62 AADR metadata field names
LAT_FIELD = "Lat."
LON_FIELD = "Long."
LOCALITY_FIELD = "Locality"
INDIVIDUAL_ID_FIELD = "Master ID"
...
def ensure_v62_npz_present() -> None:
    missing = [path for path in (AADR_V62_NPZ_PATH, AADR_V62_METADATA_PATH) if not path.is_file()]
    if missing:
        missing_str = ", ".join(str(path) for path in missing)
        raise SystemExit(
            f"Missing input files: {missing_str}. Run `pixi run fastpmr-aadr-v62` after `pixi run prepare-aadr-v62`."
        )
...
def main() -> None:
    ensure_v62_npz_present()
    samples, site_overlaps, mismatch_rates, mismatch_rates_95_ci_lower, mismatch_rates_95_ci_upper, _covered_snps = (
        load_aadr_npz_arrays(AADR_V62_NPZ_PATH)
    )
    metadata = load_aadr_metadata(AADR_V62_METADATA_PATH)
